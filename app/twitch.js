import { extend, entries } from 'lodash/object';
...
(function ($) {
    ...
    Twitch.api = function (options, callback) {
        ...
        let authenticated = !!Twitch._config.session.token,
...
(function () {
    ...
    let SESSION_KEY = 'twitch_oauth_session',
        $ = Twitch.$;
    let parseFragment = function (hash) {
        let match,
            session;
    ...
    let updateSession = function (callback) {
    ...
    let getStatus = function (options, callback) {
        ...
        let makeSession = function (session) {
    ...
    let login = function (options) {
        ...
        let url = Twitch.baseUrl + 'oauth2/authorize?' + $.param(params);
